...
import com.airbnb.lottie.model.content.ComputedGradientColor;
...
public class ComputedGradientColorParser implements ValueParser<GradientColor> {
  /**
   * The number of colors if it exists in the json or -1 if it doesn't (legacy bodymovin)
   */
  private int colorPoints;

  public ComputedGradientColorParser(int colorPoints) {
  ...
  public GradientColor parse(JsonReader reader, float scale)
    ...
    for (int i = 0; i < colorPoints * 4; i++) {
      ...
      switch (i % 4) {
        case 0:
          ...
          if (colorIndex > 0 && colorPositions[colorIndex - 1] >= (float) value) {
            colorPositions[colorIndex] = (float) value + 0.01f;
          } else {
    ...
    if (array.size() <= opacityStartIndex) {
      return new ComputedGradientColor(colorPositions, colors, null, null);
    }
    ...
    return new ComputedGradientColor(colorPositions, colors, opacityPositions, opacities);
  }
