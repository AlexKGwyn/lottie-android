...
public class PreComputedGradientColorParser implements com.airbnb.lottie.parser.ValueParser<GradientColor> {
  /**
   * The number of colors if it exists in the json or -1 if it doesn't (legacy bodymovin)
   */
  private int colorPoints;

  public PreComputedGradientColorParser(int colorPoints) {
  ...
  public GradientColor parse(JsonReader reader, float scale)
    ...
    if (array.size() <= opacityStartIndex) {
      return new PreComputedGradientColor(colorPositions, colors);
    }
    ...
    return new PreComputedGradientColor(gradientPositions, gradientColors);
  }
