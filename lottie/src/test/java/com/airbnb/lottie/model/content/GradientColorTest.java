...
public class GradientColorTest extends BaseTest {
  ...
  public void testSimpleGradientMerge(){
    ...
    GradientColor gradientColor = new ComputedGradientColor(colorPositions, colors, opacityPositions, opacities);
  ...
  public void testOffsetOpacityGradientMerge(){
    ...
    GradientColor gradientColor = new ComputedGradientColor(colorPositions, colors, opacityPositions, opacities);
    assertArrayEquals(gradientColor.getColors(), new int[]{0xFF000000, 0x7FBCBCBC, 0x00FFFFFF});
    assertArrayEquals(gradientColor.getPositions(), new float[]{0, .5f, 1}, .01f);
  }
  ...
  public void testOffsetColorGradientMerge(){
    ...
    float[] opacities = {1, 0};
    GradientColor gradientColor = new ComputedGradientColor(colorPositions, colors, opacityPositions, opacities);
    assertArrayEquals(gradientColor.getColors(), new int[]{0xFF000000, 0x7FFFFFFF, 0x00000000});
    assertArrayEquals(gradientColor.getPositions(), new float[]{0, .5f, 1}, .01f);
  }
}
